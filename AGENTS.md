...
- 概念図は Mermaid のコードブロックで書く。図中の数式は Mermaid の数式記法で本文の KaTeX と字形をそろえる（`astro.config.mjs`で`forceLegacyMathML`を有効化済み）。次の規約に従う。
  ...
  - 根号（`\sqrt`）・オーバーブレースなどの伸縮グリフも使える。KaTeX はそれらを高さ指定付きの SVG で描き、foreignObject 内では高さ 0 に潰れる。`src/styles/global.css`で`.mermaid .katex svg`に`height: 100%`を与えて復元している。新たな伸縮グリフを使うときは、ビルドして実機で確認する。
