 ...
 * 座標計算には D3 の `scaleLinear`、曲線の経路生成には `d3-shape` の `line` を用いる。
...
type FunctionGraphProps = {
  ...
  equalScale?: boolean;
  /**
   * 凡例（色見本＋数式ラベル）の位置。既定は "auto"（曲線・点・漸近線が最も少ない隅を
   * 自動で選ぶ）。曲線ラベルを線のそばに置くと複数曲線・点ラベルと重なるため、
   * ラベル付きの曲線は凡例にまとめる。"none" で凡例を描かない。
   */
  legend?: "auto" | "top-left" | "top-right" | "bottom-left" | "bottom-right" | "none";
  /** 支援技術が読み上げるアクセシブルな説明。 */
  ariaLabel: string;
};
