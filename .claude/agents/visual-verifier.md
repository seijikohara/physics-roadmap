...
- **2D 図**（関数・幾何・ベクトル・ベン図・数直線）は、ビルド時生成の静的 SVG を出力する宣言的 React コンポーネントで表現する。コンポーネントは`src/components/`にあり、`FunctionGraph`・`Triangle`・`UnitCircle`・`NumberLine`・`AreaModel`・`VennDiagram`・`SpaceVectorDiagram`などを使う。標準ライブラリは visx（`@visx/*`）とし、クライアント JS を載せない。
- **概念図・フロー**は`FlowChart`コンポーネント（`@dagrejs/dagre` ＋ `@visx/network`）で表現する。ビルド時生成の静的 SVG で、ラベル中の数式は`FlowChart`自身が`katex.renderToString`で組版し、地の文と字形をそろえる。Mermaid は使わない。
- **数式**は KaTeX で表現する。
- **データグラフ**は visx によるビルド時生成の静的 SVG で表現する。
...
for (const route of routes) {
  ...
  if (figs.length === 0) {
    console.log(`${name}: 0 figures (クライアントアイランドは live 経路で確認)`);
    continue;
  }
...
### 3. クライアントアイランドは live ページで確認する

`FlowChart`を含むすべての図はビルド時生成の静的 SVG で、SSR HTML に現れるため、コンタクトシート（手段 2）で確認できる。一方、スライダー・ドラッグなど JS で描画されるクライアントアイランドは`curl`の SSR HTML に最終形が無い。JS で描画される図は、Chrome に URL を直接読ませて JS を実行させ、ページ全体を撮る。
...
撮れた`page.png`を Read し、字形・矢印の向き・はみ出しを確認する。ページが縦に長く撮りきれない場合は、`--window-size`の高さを増やすか、確認したい図の位置で分割して撮る。
...
- **ラベルの書式（ソース）**: 図コンポーネントの`label`/`tex`で、数式でない日本語を`\text{}`で囲んでいないか。日本語は素で書く（`"\\text{隣辺}"`ではなく`"隣辺"`）。`$...$`の内側に日本語を`\text{}`で押し込んでいる箇所も、日本語を`$...$`の外へ出すよう指摘する。レンダリング画像では見分けにくいため、ソースを読んで確認する。
...
追加を推奨する図ごとに、実現手段（既存コンポーネントの流用・`FlowChart`・新規コンポーネント・数式ブロック）と新規実装の要否を示す。新規コンポーネントが要る場合は、章側で作らず`docs/superpowers/specs/2026-06-24-visualization-stack-design.md`の方針に沿って追加を相談する旨を併記する。
